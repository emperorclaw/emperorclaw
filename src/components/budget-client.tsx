                    ...
                    <tbody className="divide-y divide-zinc-800/50">
                        {agents.map(a => {
                            ...
                            return (<tr key={a.id} className="hover:bg-zinc-900/50 transition-colors">
                                ...
                                <td className="px-5 py-3 text-right">{budget <= 0 ? <span className="text-zinc-500 text-xs">—</span> : a.budgetStatus === "paused" ? <span className="text-rose-400 text-xs font-medium bg-rose-500/10 px-2 py-0.5 rounded">Paused</span> : a.budgetStatus === "warning" ? <span className="text-amber-400 text-xs font-medium bg-amber-500/10 px-2 py-0.5 rounded">⚠ {Math.round(pct)}%</span> : <span className="text-emerald-400 text-xs font-medium bg-emerald-500/10 px-2 py-0.5 rounded">{Math.round(pct)}%</span>}</td>
                            </tr>);
                        })}
                    </tbody></table></div>
