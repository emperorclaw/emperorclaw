                    ...
                    <tbody className="divide-y divide-zinc-800/50">
                        {agents.map(a => {
                            ...
                            return (<tr key={a.id} className="hover:bg-zinc-900/50 transition-colors">
                                ...
                                <td className="px-5 py-3 text-right">{budget <= 0 ? <span className="text-zinc-500 text-xs">—</span> : a.budgetStatus === "paused" ? <button onClick={async () => { await fetch(`/api/agents/${a.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ budgetStatus: "active" }) }); refresh(); }} className="text-rose-400 text-xs font-medium bg-rose-500/10 px-2 py-0.5 rounded hover:bg-rose-500/20 cursor-pointer" title="Budget exhausted. Click to reactivate.">⏸ Paused</button> : a.budgetStatus === "warning" ? <button onClick={async () => { await fetch(`/api/agents/${a.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ budgetStatus: "active" }) }); refresh(); }} className="text-amber-400 text-xs font-medium bg-amber-500/10 px-2 py-0.5 rounded hover:bg-amber-500/20 cursor-pointer" title="Nearing budget limit. Click to dismiss warning.">⚠ {Math.round(pct)}%</button> : <span className="text-emerald-400 text-xs font-medium bg-emerald-500/10 px-2 py-0.5 rounded">{Math.round(pct)}%</span>}</td>
                            </tr>);
                        })}
                    </tbody></table></div>
