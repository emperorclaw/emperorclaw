...
import { buildResourceChildPath, buildResourceFolderTree, normalizeResourcePath, resourcePathAncestors, type ResourceFolderNode } from "@/lib/resource-paths";
...
// A scope key uniquely identifies where a note (or folder) lives:
// "company", "customer:<id>", "project:<id>", "agent:<id>". Folders are scoped —
// "Fundraising" under Company and under an Agent are independent trees.
function scopeKeyOf(scopeType: string, scopeId: string | null): string {
  return scopeType === "company" ? "company" : `${scopeType}:${scopeId || "unknown"}`;
}
function parseScopeKey(key: string): { scopeType: string; scopeId: string | null } {
  const idx = key.indexOf(":");
  if (idx < 0) return { scopeType: key, scopeId: null };
  const id = key.slice(idx + 1);
  return { scopeType: key.slice(0, idx), scopeId: id && id !== "unknown" ? id : null };
}
// Collapse-state key for a folder within a scope (paths repeat across scopes).
function folderKey(scopeKey: string, path: string): string {
...
export default function ResourcesClient({
  ...
  // Which scope the New-folder dialog / next new note targets.
  const [newFolderScope, setNewFolderScope] = useState<string>("company");
  const [renameFolderTarget, setRenameFolderTarget] = useState<{ scopeKey: string; path: string } | null>(null);
  const [renameFolderValue, setRenameFolderValue] = useState("");
  const [deleteFolderTarget, setDeleteFolderTarget] = useState<{ scopeKey: string; path: string } | null>(null);
  ...
  // One unified tree per scope: scope → folders (nested) → notes. Folders are
  // scoped, so the same path under Company and under an Agent are independent.
  const scopeGroups = useMemo(() => {
    const customerNames = new Map(customers.map((customer) => [customer.id, customer.name]));
    const projectNames = new Map(projects.map((project) => [project.id, project.name]));
    const agentNames = new Map(agents.map((agent) => [agent.id, agent.name]));
    const labelFor = (scopeType: string, scopeId: string | null) => {
      if (scopeType === "company") return "Company";
      if (scopeType === "customer") return customerNames.get(scopeId || "") || "Unknown customer";
      if (scopeType === "project") return projectNames.get(scopeId || "") || "Unknown project";
      if (scopeType === "agent") return agentNames.get(scopeId || "") || "Unknown agent";
      return scopeLabel(scopeType);
    };

    type Group = { key: string; label: string; scopeType: string; scopeId: string | null; notes: ResourceRecord[] };
    const byScope = new Map<string, Group>();
    const ensure = (scopeType: string, scopeId: string | null) => {
      const key = scopeKeyOf(scopeType, scopeId);
      if (!byScope.has(key)) byScope.set(key, { key, label: labelFor(scopeType, scopeId), scopeType, scopeId, notes: [] });
      return byScope.get(key)!;
    };

    for (const resource of filteredResources) {
      ensure(resource.scopeType, resource.scopeType === "company" ? null : resource.scopeId).notes.push(resource);
    }
    // Scopes that only hold a just-created (still empty) folder must still appear.
    for (const pf of pendingFolders) {
      const { scopeType, scopeId } = parseScopeKey(pf.scopeKey);
      ensure(scopeType, scopeId);
    }
    // Company always shows, so there is always somewhere to create the first note.
    ensure("company", null);

    const rank = (key: string) => key === "company" ? 0 : key.startsWith("customer:") ? 1 : key.startsWith("project:") ? 2 : key.startsWith("agent:") ? 3 : 4;

    return Array.from(byScope.values()).map((group) => {
      const extraPaths = pendingFolders.filter((pf) => pf.scopeKey === group.key).map((pf) => pf.path);
      const tree = buildResourceFolderTree(group.notes, extraPaths);
      const notesByFolder = new Map<string, ResourceRecord[]>();
      for (const note of group.notes) {
        const path = normalizeResourcePath(note.path);
        if (!notesByFolder.has(path)) notesByFolder.set(path, []);
        notesByFolder.get(path)!.push(note);
      }
      for (const list of notesByFolder.values()) {
        list.sort((a, b) => (a.displayName || a.name).localeCompare(b.displayName || b.name));
      }
      return { ...group, tree, notesByFolder, rootNotes: notesByFolder.get("") || [] };
    }).sort((a, b) => rank(a.key) - rank(b.key) || a.label.localeCompare(b.label));
  }, [agents, customers, filteredResources, pendingFolders, projects]);
  ...
  function openNewFolderDialog(scopeKey: string, parentPath: string) {
    setNewFolderScope(scopeKey);
    setNewFolderParentPath(normalizeResourcePath(parentPath));
    setNewFolderName("");
    setNewFolderOpen(true);
  }

  function createFolder() {
    const path = buildResourceChildPath(newFolderParentPath, newFolderName);
    if (!path) return toast.error(newFolderName.includes("/") ? "Folder names cannot contain /" : "Enter a folder name");
    const scopeKey = newFolderScope || "company";

    // A folder only becomes real once a note lives in it; hold it (with its
    // scope) until then. It appears in the tree WITHOUT hiding anything else.
    ...
    toast.success(`Folder ${path} ready — right-click it to add a note`);
  }

  async function renameFolder() {
    const target = renameFolderTarget;
    const toPath = normalizeResourcePath(renameFolderValue);
    if (!target || !toPath || target.path === toPath) { setRenameFolderTarget(null); return; }
    const { scopeKey, path: fromPath } = target;

    const response = await fetch("/api/resources/folders", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ fromPath, toPath, ...parseScopeKey(scopeKey) }),
    });
    const body = await response.json();
    if (!response.ok) { setRenameFolderTarget(null); return toast.error(body.error || "Failed to rename folder"); }

    // Re-file only this scope's notes so the tree updates without a reload.
    setResources((current) => current.map((resource) => {
      if (scopeKeyOf(resource.scopeType, resource.scopeType === "company" ? null : resource.scopeId) !== scopeKey) return resource;
    ...
    setPendingFolders((current) => current.map((pf) => (pf.scopeKey === scopeKey && pf.path === fromPath ? { ...pf, path: toPath } : pf)));
  ...
  async function deleteFolder() {
    ...
    if (!target) return;
    const { scopeKey, path } = target;
    const inFolder = (p: string) => p === path || p.startsWith(`${path}/`);

    // Empty (pending-only) folder — just drop it locally.
    const group = scopeGroups.find((g) => g.key === scopeKey);
    const noteCount = group ? group.notes.filter((n) => inFolder(normalizeResourcePath(n.path))).length : 0;
    if (noteCount === 0) {
      setPendingFolders((current) => current.filter((pf) => !(pf.scopeKey === scopeKey && inFolder(pf.path))));
      setDeleteFolderTarget(null);
      toast.success(`Folder ${path} removed`);
      return;
    }
    ...
    const response = await fetch("/api/resources/folders", {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ path, ...parseScopeKey(scopeKey) }),
    });
    const body = await response.json();
    if (!response.ok) { setDeleteFolderTarget(null); return toast.error(body.error || "Failed to delete folder"); }

    setResources((current) => current.filter((resource) => {
      if (scopeKeyOf(resource.scopeType, resource.scopeType === "company" ? null : resource.scopeId) !== scopeKey) return true;
      return !inFolder(normalizeResourcePath(resource.path));
    }));
    setPendingFolders((current) => current.filter((pf) => !(pf.scopeKey === scopeKey && inFolder(pf.path))));
    if (selectedResource && inFolder(normalizeResourcePath(selectedResource.path)) &&
        scopeKeyOf(selectedResource.scopeType, selectedResource.scopeType === "company" ? null : selectedResource.scopeId) === scopeKey) {
      setSelectedResourceId(null);
    }
    setDeleteFolderTarget(null);
    toast.success(`Deleted folder ${path} (${body.deleted ?? noteCount} notes)`);
  }

  function renderNoteRow(note: ResourceRecord, depth: number): ReactNode {
    return (
      <ContextMenu key={note.id}>
        <ContextMenuTrigger asChild>
          <button
            onClick={() => setSelectedResourceId(note.id)}
            style={{ paddingLeft: `${depth * 12 + 8}px` }}
            className={cn("group flex w-full cursor-pointer items-center gap-1.5 rounded-md py-1.5 pr-2 text-left text-sm transition-colors", selectedResourceId === note.id ? "bg-cyan-400/10 text-cyan-100" : "text-zinc-400 hover:bg-zinc-900 hover:text-zinc-100")}
          >
            <IconFileText className={cn("h-3.5 w-3.5 shrink-0", selectedResourceId === note.id ? "text-cyan-300" : "text-zinc-600 group-hover:text-zinc-400")} />
            <span className="min-w-0 flex-1 truncate">{note.displayName || note.name}</span>
            {noteStatus(note.configText || "") === "draft" && <span className="rounded bg-amber-500/10 px-1.5 py-0.5 text-[10px] font-medium uppercase tracking-wider text-amber-300">draft</span>}
            {note.isShared && <span className="h-1.5 w-1.5 rounded-full bg-cyan-400" title="Shared with agents" />}
          </button>
        </ContextMenuTrigger>
        <ContextMenuContent className="w-48 border-zinc-800 bg-zinc-950 text-zinc-100">
          <ContextMenuItem onClick={() => setSelectedResourceId(note.id)}>
            <IconFileText className="h-3.5 w-3.5" /> Open
          </ContextMenuItem>
          <ContextMenuItem variant="destructive" onClick={() => setDeleteDialogTarget(note)}>
        ...
        <ContextMenu>
          <ContextMenuTrigger asChild>
            <div
              role="button"
              tabIndex={0}
              onClick={() => toggleFolder(group.key, node.path)}
              onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); toggleFolder(group.key, node.path); } }}
              style={{ paddingLeft: `${depth * 12 + 8}px` }}
              className="group flex w-full cursor-pointer items-center gap-1.5 rounded-md py-1.5 pr-2 text-left text-sm text-zinc-400 transition-colors hover:bg-zinc-900 hover:text-zinc-100"
            >
              <span className="rounded p-0.5 text-zinc-500">
                {isCollapsed ? <IconChevronRight className="h-3 w-3" /> : <IconChevronDown className="h-3 w-3" />}
              </span>
              {isCollapsed ? <IconFolder className="h-3.5 w-3.5 shrink-0 text-amber-300/70" /> : <IconFolderOpen className="h-3.5 w-3.5 shrink-0 text-amber-300/70" />}
          ...
          <ContextMenuContent className="w-52 border-zinc-800 bg-zinc-950 text-zinc-100">
            <ContextMenuItem onClick={() => createResource(group.key, node.path)}>
              <IconPlus className="h-3.5 w-3.5" /> New note here
            </ContextMenuItem>
            <ContextMenuItem onClick={() => openNewFolderDialog(group.key, node.path)}>
              <IconFolderPlus className="h-3.5 w-3.5" /> New subfolder
            </ContextMenuItem>
            <ContextMenuItem onClick={() => { setRenameFolderTarget({ scopeKey: group.key, path: node.path }); setRenameFolderValue(node.path); }}>
              <IconPencil className="h-3.5 w-3.5" /> Rename / move
            </ContextMenuItem>
            <ContextMenuItem variant="destructive" onClick={() => setDeleteFolderTarget({ scopeKey: group.key, path: node.path })}>
              <IconTrash className="h-3.5 w-3.5" /> Delete folder
            </ContextMenuItem>
          </ContextMenuContent>
        </ContextMenu>
        {!isCollapsed && (
          <>
            {node.children.map((child) => renderFolderNode(group, child, depth + 1))}
  ...
  async function createResource(scopeKey: string = "company", path: string = "") {
    const { scopeType, scopeId } = parseScopeKey(scopeKey);
    const cleanPath = normalizeResourcePath(path);
  ...
  const newFolderScopeLabel =
    scopeGroups.find((group) => group.key === newFolderScope)?.label ||
    scopeLabel(parseScopeKey(newFolderScope).scopeType);
  const newFolderLocation = [newFolderScopeLabel, newFolderParentPath].filter(Boolean).join(" / ");
  ...
  return (
    <div className="mx-auto flex h-[calc(100vh-6rem)] sm:h-[calc(100vh-8rem)] max-w-[1800px] flex-col gap-4 animate-in fade-in duration-500">
        ...
        <div className="grid grid-cols-1 border-b border-zinc-800 bg-zinc-950/95 lg:grid-cols-[300px_minmax(0,1fr)_360px]">
          <div className="flex h-11 items-center justify-between border-b border-zinc-800 px-3 lg:border-b-0 lg:border-r">
            <div className="flex items-center gap-1.5 text-zinc-500">
              <button className="cursor-pointer rounded p-1 transition-colors hover:bg-zinc-900 hover:text-zinc-200" title="New note" aria-label="New note" onClick={() => void createResource("company", "")}><IconFileText className="h-4 w-4" /></button>
              <button className="cursor-pointer rounded p-1 transition-colors hover:bg-zinc-900 hover:text-zinc-200" title="New folder" aria-label="New folder" onClick={() => openNewFolderDialog("company", "")}><IconFolderPlus className="h-4 w-4" /></button>
                      ...
                      <ContextMenu>
                        <ContextMenuTrigger asChild>
                          <div
                            role="button"
                            tabIndex={0}
                            onClick={() => toggleScope(group.key)}
                            onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); toggleScope(group.key); } }}
                            className="group flex w-full cursor-pointer items-center gap-1.5 rounded-md px-1.5 py-1 text-[11px] font-semibold uppercase tracking-wider text-zinc-400 transition-colors hover:bg-zinc-900 hover:text-zinc-200"
                          >
                            {scopeCollapsed ? <IconChevronRight className="h-3.5 w-3.5" /> : <IconChevronDown className="h-3.5 w-3.5" />}
                            <span className="min-w-0 flex-1 truncate">{group.label}</span>
                            <button
                              type="button"
                              aria-label={`New folder in ${group.label}`}
                              onClick={(event) => { event.stopPropagation(); openNewFolderDialog(group.key, ""); }}
                              className="rounded p-0.5 text-zinc-600 opacity-0 transition-opacity hover:bg-zinc-800 hover:text-zinc-200 group-hover:opacity-100"
                            >
                              <IconFolderPlus className="h-3.5 w-3.5" />
                            </button>
                            <span className="text-zinc-700">{group.notes.length}</span>
                          </div>
                        </ContextMenuTrigger>
                        <ContextMenuContent className="w-52 border-zinc-800 bg-zinc-950 text-zinc-100">
                          <ContextMenuItem onClick={() => createResource(group.key, "")}>
                            <IconPlus className="h-3.5 w-3.5" /> New note
                          </ContextMenuItem>
                          <ContextMenuItem onClick={() => openNewFolderDialog(group.key, "")}>
                            <IconFolderPlus className="h-3.5 w-3.5" /> New folder
                          </ContextMenuItem>
                        </ContextMenuContent>
                      </ContextMenu>
                      {!scopeCollapsed && (
                        <div className="mt-0.5 space-y-0.5">
                          {group.tree.map((node) => renderFolderNode(group, node, 1))}
                          {group.rootNotes.map((note) => renderNoteRow(note, 1))}
                          {isEmpty && <p className="py-1.5 pl-6 text-[11px] text-zinc-600">No notes yet. Right-click to add one.</p>}
                        </div>
                      )}
                    </div>
                  );
                })}
                {!scopeGroups.length && <EmptyState>No notes yet.</EmptyState>}
              </div>
            </div>
          </aside>
        ...
        <DialogContent className="border-zinc-800 bg-zinc-950 text-zinc-100 sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription className="text-zinc-400">
              Create a folder inside the location you selected.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="space-y-1.5">
              <span className="text-[11px] font-medium uppercase tracking-wider text-zinc-500">Location</span>
              <div className="flex min-h-9 items-center gap-2 rounded-md border border-zinc-800 bg-zinc-900/70 px-3 text-sm text-zinc-300">
                <IconFolder className="h-4 w-4 shrink-0 text-amber-300/70" />
                <span className="truncate">{newFolderLocation}</span>
              </div>
            </div>
            <label className="block space-y-1.5">
              <span className="text-[11px] font-medium uppercase tracking-wider text-zinc-500">Folder name</span>
              <input
                autoFocus
                value={newFolderName}
                ...
                spellCheck={false}
                className="h-10 w-full rounded-md border border-zinc-800 bg-zinc-950 px-3 text-sm text-zinc-100 outline-none placeholder:text-zinc-600 focus:border-cyan-400/60 focus:ring-2 focus:ring-cyan-400/10"
              />
            </label>
            <p className="text-xs leading-5 text-zinc-500">
              The folder stays visible while you work and becomes durable when a note is filed inside it.
            </p>
          </div>
          <DialogFooter>
      ...
      <Dialog open={Boolean(renameFolderTarget)} onOpenChange={(open) => { if (!open) setRenameFolderTarget(null); }}>
        <DialogContent className="border-zinc-800 bg-zinc-950 text-zinc-100 sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription className="text-zinc-400">
              Every note under <span className="text-zinc-200">{renameFolderTarget?.path}</span> is re-filed.
      ...
      <Dialog open={Boolean(deleteFolderTarget)} onOpenChange={(open) => { if (!open) setDeleteFolderTarget(null); }}>
        <DialogContent className="border-zinc-800 bg-zinc-950 text-zinc-100 sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Delete folder &ldquo;{deleteFolderTarget?.path}&rdquo;?</DialogTitle>
            <DialogDescription className="text-zinc-400">
              This permanently deletes the folder and <span className="text-zinc-200">every note inside it and its subfolders</span>. This cannot be undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <button onClick={() => setDeleteFolderTarget(null)} className="cursor-pointer rounded-md border border-zinc-800 px-3 py-2 text-xs font-semibold text-zinc-300 hover:bg-zinc-900">Cancel</button>
            <button onClick={() => void deleteFolder()} className="cursor-pointer rounded-md border border-red-500/40 bg-red-500/10 px-3 py-2 text-xs font-semibold text-red-300 hover:bg-red-500/15">Delete folder</button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
